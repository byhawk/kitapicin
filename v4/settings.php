...
class SiteSettings {
    ...
    public function loadDefaultSettings($force = false) {
        $defaults = [
        ...
        try {
            foreach ($defaults as $key => $value) {
                try {
                    // Force parametresi true ise mevcut ayarları da güncelle
                    if ($force || !$this->getSetting($key)) {
                        $result = $this->updateSetting($key, $value, "Varsayılan ayar: $key");
                        if ($result) {
                            $success_count++;
                        } else {
                            $error_count++;
                            error_log("Varsayılan ayar eklenemedi: $key");
                        }
                    }
                } catch (Exception $e) {
                    $error_count++;
                    error_log("Varsayılan ayar hatası ($key): " . $e->getMessage());
                }
            }
            
            error_log("Varsayılan ayarlar yüklendi: $success_count başarılı, $error_count hatalı");
            return ['success' => $success_count, 'errors' => $error_count];
            
        } catch (Exception $e) {
            error_log("Varsayılan ayarlar genel hatası: " . $e->getMessage());
            return false;
        }
    }
}
